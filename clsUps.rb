...
class SMDUPS
 attr_accessor(:doc, :file_string, :tracking_nums, :status_codes, :filename, :filepath, :savefile)
  ...
  def initialize
    #status codes are case dependant
    @status_codes = Hash["In Transit", 1, "In Transit - On Time", 1, "Delivered", 2, "Exception", 3, "Pickup", 4, "Manifest Pickup", 5, "Bad Number", 6, "Not Available", 7, "Billing Information Received", 8, "Returned to shipper", 9, "Voided Information Received", 10, "Out For Delivery", 11, "Package was driver released", 2, "Signature obtained", 2, "Left at customer's location", 2, "Origin Scan", 1]
    @doc = ""
    @file_string = ""
    #@filepath = "C:/Ops/Customers/"
    @filepath = "/home/salman/Downloads/ups/"
    @savefile = 1
  end
  ...
  def get_tracking_nums()
   ...
   def testit
     ...
     process_tracking_nums("1Z68585W0351047551","","Smith, John")
     #save_results()
  end
  ...
  def parse_results(trackingNums)
    ...
    #searching for return code errors - cant find tracking number
    ...
    #*************************** HERE'S THE PROBLEM
    #searching for return code status 
    if @doc.search("table[@class=dataTable]").length > 0
      ...
      #puts "Status: " + status
      @file_string << get_result_string(status.upcase) + "\n"
      @savefile = 1
    end
  end
  ...
  def get_result_string(tdstring)
    ...
    @status_codes.each_key do |key|
      #puts key
      if tdstring.include?(key.upcase)
        #puts key.to_s
        return key.to_s
      end
    end
